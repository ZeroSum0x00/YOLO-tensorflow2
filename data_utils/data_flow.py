...
class Train_Data_Sequence(Sequence):
    def __init__(self, 
                 ...
                 anchors_mask=cfg.YOLO_ANCHORS_MASK,
                 max_bboxes_per_scale=cfg.YOLO_MAX_BBOX_PER_SCALE,
                 augmentor=cfg.AUGMENTATION['train'],
                 normalizer=cfg.NORMALIZER):
        ...
        if isinstance(augmentor, str):
            self.augmentor = Augmentor(target_size=target_size, max_bboxes_per_scale=max_bboxes_per_scale, aug_mode=augmentor)
        else:
        ...
        self.normalizer = Normalizer(max_bboxes_per_scale=max_bboxes_per_scale, mode=normalizer)

        self.yolo_strides = np.array(yolo_strides if yolo_strides else cfg.YOLO_STRIDES)
        self.num_classes = len(classes if classes else cfg.OBJECT_CLASSES)
...
#             batch_image[i] = image
        ...
        batch_label = preprocess_true_boxes(batch_label, self.target_size, self.anchors, self.anchors_mask, self.num_classes, self.yolo_strides)
...
class Valid_Data_Sequence(Sequence):
    def __init__(self, 
                 ...
                 anchors_mask=cfg.YOLO_ANCHORS_MASK,
                 max_bboxes_per_scale=cfg.YOLO_MAX_BBOX_PER_SCALE,
                 augmentor=cfg.AUGMENTATION['validation'],
                 normalizer=cfg.NORMALIZER):
        ...
        if isinstance(augmentor, str):
            self.augmentor = Augmentor(max_bboxes_per_scale=max_bboxes_per_scale, aug_mode=augmentor)
        else:
            self.augmentor = augmentor
...
class Test_Data_Sequence(Sequence):
    def __init__(self, 
                 ...
                 anchors_mask=cfg.YOLO_ANCHORS_MASK,
                 max_bboxes_per_scale=cfg.YOLO_MAX_BBOX_PER_SCALE,
                 augmentor=cfg.AUGMENTATION['test'],
                 normalizer=cfg.NORMALIZER):
